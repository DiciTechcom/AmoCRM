...
class DomainRequest extends EventEmitter {
  ...
  request(url, data = {}, method = 'GET', options = {}) {
    ...
    const headers = this.getRequestHeaders(url, encodedData, method, options)
    const request = this.createRequest(url, encodedData, method, headers)
  ...
  getRequestHeaders(url, encodedData = '', method = 'GET', options = {}) {
    const isGET = method === 'GET'
    const headers = this.getDefaultHeaders(options)
  ...
  createRequest(url, encodedData = '', method = 'GET', headers = {}) {
    const isGET = method === 'GET'
    const path = isGET ? `${url}?${encodedData}` : url
